...
- 已新增 `0004_role_visibility_calendar` migration，补充 `guide_availability_windows`、`route_follow_requests`，并为 `travel_plans` 增加 `looking_for_partner` 和 `partner_note`。
- 已完成角色隔离地图 / 日历后端基础接口：`GET /api/v1/markets/{market_id}/map-routes` 和 `GET /api/v1/markets/{market_id}/calendar-events`，调用方必须传入 `traveler` 或 `guide` 视角。
- 已完成导游发现旅客计划的隐私切片接口：`GET /api/v1/markets/{market_id}/travel-plan-leads`，只返回与导游服务地区和可服务时间匹配的城市片段，不暴露旅客完整后续路线。
- 已完成旅行者寻找旅伴的重叠行程接口：`GET /api/v1/markets/{market_id}/partner-leads`，只在双方旅行计划都开启 `looking_for_partner` 且城市日期重叠时返回对方。
- 已新增 `0005_route_service_modes` migration，补充旅行计划 `guide_hiring_mode`、行程节点 `place_name` / `looking_for_partner`、导游资料 `service_scope_modes`。
- 已完成两种招揽导游方式的数据协议：`point_to_point` 表示按单一地区 / 片段匹配导游，`full_route` 表示整条线路由同一导游服务。
- 已完成导游服务方式配置协议：导游资料可保存 `point_to_point`、`full_route` 或两者都支持。
- 已更新导游发现旅客计划的匹配条件：点对点计划要求导游支持单一地区且匹配该节点城市与时间；整条线路计划要求导游支持整条路线，并且当前基础规则要求所有 route nodes 都落在导游服务地区和可服务时间窗内。
...
- `route_follow_requests` 目前只是数据库结构预留；路线跟随的申请、双方确认、价格重算、导游跨城市熟悉度评估和风控规则暂缓。
- 导游可服务日历目前只记录可服务时间窗和已预约订单；冲突校验、重复预约阻止、跨地区服务能力证明、可服务状态批量编辑暂缓。
- 旅伴匹配目前只做双方主动开启和城市日期重叠；旅伴资料可见字段、聊天入口、屏蔽 / 举报、未成年人保护、同性 / 语言 / 兴趣筛选等隐私规则暂缓。
- 整条线路导游服务目前只做基础地区和时间覆盖校验；导游跨城市资质、城市熟悉度评分、路线中交通陪同责任、跨地价格计算和保险 / 法律责任暂缓。
...
- 如果要继续实现路线跟随，必须先确认导游跨城市服务能力评估规则、旅客分享后续行程的授权范围、跟随订单的重新报价和取消责任。
- 如果要继续实现旅伴功能，必须先确认旅伴卡片可见字段、聊天发起限制、举报和拉黑策略，以及是否需要额外的用户安全提示。
- 如果要继续实现整条线路导游，必须先确认导游在非注册服务地区的资质证明、平台审核标准、整条路线报价方式、跨城市取消规则和责任边界。
...
- 旅行计划创建不再使用单行“旅行路线”文本框，必须通过“添加地点”逐个添加城市 / 地区；每个地点需要到达日期和离开日期，可单独勾选 looking for partner；人数和预算必填，接 / 送和备注不是必填。
- 旅行计划必须选择招揽导游方式：点对点或整条线路同一导游。
- 导游在“我的服务”中可以配置自己接受单一地区服务、整条路线服务，或两者都接受。
...
- 已完成 App 顶层身份切换接入 `/api/v1/me/role-switch`，切换后按新身份刷新 bootstrap 数据；切换失败时显示 API 错误并保持当前身份，避免前端伪造未开通身份。
...
- 已完成旅行计划创建表单校验：至少一个地点节点、每个节点的城市 / 地区、到达日期、离开日期、人数和预算必填；接 / 送和备注选填。
- 已完成旅行计划创建低风险 API 接入：先创建 draft，再逐个创建 route node，发布时再调用 publish；route node 会保存地点名称、到达 / 离开时间和节点级 looking for partner。
- 已完成旅行计划招揽导游方式 UI：点对点和整条线路二选一，并写入后端 `guide_hiring_mode`。
- 已完成导游“我的服务”页服务方式展示和编辑：可配置单一地区、整条路线或两者都接受，并写入后端 `service_scope_modes`。
- 已完成 DiscoverPage 的角色隔离读取：旅行者视角只显示导游列表和旅伴入口；导游视角只显示与服务地区和可服务时间匹配的旅客计划片段。
- 已完成 MapView 的角色隔离展示：旅行者视角只显示本人作为旅客参与的订单路线和旅行者日历；导游视角只显示本人作为导游参与的订单路线、可服务时间和导游日历。
- 已完成基础旅伴页签：只展示双方都开启 `looking_for_partner` 且城市日期重叠的旅行者，不展示对方完整后续行程。

未完成 / 暂缓项：

- 地图热度、推荐城市、路线热度、搜索排序算法和完整匹配算法仍作为 `memory / 延后实现项` 暂缓。
- DiscoverPage 的完整筛选、排序、价格换算和导游上架规则暂缓；M9 只接导游列表基础读取。
- 发现页导游视角目前只展示匹配片段，尚未实现接单申请、报价、计划片段详情页和导游不可见字段的细粒度审计。
- 旅伴页目前只展示重叠城市和日期；旅伴聊天、屏蔽、举报、资料可见范围、兴趣筛选和安全提示暂缓。
- 路线跟随目前只有数据库预留，未实现前端入口、双方确认流程、导游跨城市能力评估和订单价格重算。
- 旅行计划路线中的城市与 region 精准映射如果缺少后端 region id，M9 允许先使用首个可用 region 或保留表单数据，后续再完善地区选择器。
- 旅行计划每个城市的到达 / 离开日期已具备基础节点式创建；节点编辑、节点拖拽排序、交通方式、停留时长、机场 / 车站细分和跨市场地点搜索仍作为 `memory / 延后实现项` 暂缓。
- 整条线路服务目前只在发现匹配中做基础过滤；整条线路下单、报价、取消规则和导游跨城资质审核暂缓。
...
- 如果发现页继续做“导游接单旅客片段”，必须先确认导游能看到的旅客资料字段、发起聊天 / 报价的权限、旅客是否需要确认被联系，以及计划片段被接单后的状态流。
- 如果旅伴功能继续推进，必须先确认旅伴资料展示边界、聊天入口、举报拉黑、安全提示和是否允许旅行者隐藏头像 / 姓名。
- 如果路线跟随继续推进，必须先确认旅客授权分享后续路线的范围、导游跨地区能力评估、跟随订单价格、取消规则和平台责任边界。
- 如果整条线路服务继续推进到订单和支付，必须先确认整条线路报价模型、服务范围证明、跨城市交通责任、导游住宿 / 交通费用是否计入订单、以及与点对点订单的状态差异。
